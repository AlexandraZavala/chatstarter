...
export const list = authenticatedQuery({
  handler: async (ctx) => {
    ...
    return await Promise.all(
      directMessage.map((dm) => getDirectMessage(ctx, dm.directMessage))
    );
  },
});
...
export const get = authenticatedQuery({
  ...
  handler: async (ctx, { id }) => {
    const member = await ctx.db
      .query("directMessageMembers")
      .withIndex("by_direct_messages_user", (q) =>
...
export const create = authenticatedMutation({
  ...
  handler: async (ctx, { username }) => {
    ...
    const directMessagesForOtherUser = await ctx.db.query("directMessageMembers").withIndex("by_user", (q) => q.eq("user", user._id)).collect();
...
const getDirectMessage = async (
  ...
  const otherMember = await ctx.db
    .query("directMessageMembers")
    .withIndex("by_direct_messages", (q) => q.eq("directMessage", id))
